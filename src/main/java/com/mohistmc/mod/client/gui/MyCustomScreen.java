...
public class MyCustomScreen extends EnhancedScreen {
    ...
    protected void buildWidgets() {
        ...
        var infoPanel = new Panel(0, 0, 0, topH, ColorUtil.fromHex("#2D2D2D"))
                .setBorder(ColorUtil.fromHex("#CCCCCC"), 2).setAlpha(0.85f).setFlexGrow(1)
        ...
        var actionPanel = new Panel(0, 0, 0, topH, ColorUtil.fromHex("#3D1F2A"))
                .setBorder(ColorUtil.fromHex("#9d2933"), 2).setAlpha(0.85f).setFlexGrow(1)
        ...
        var topRemaining = new Panel(0, 0, 0, topH, ColorUtil.fromHex("#1A1A2E"))
                .setAlpha(0.5f).setFlexGrow(1).setBorder(ColorUtil.fromHex("#888888"), 1)
        ...
        list.addItem(new SliderItem(Component.literal("音量"), 0.7f).setFillColor(0xFF2196F3)
                .onChange(v -> System.out.println("音量: " + (int)(v * 100) + "%")));
        list.addItem(new SliderItem(Component.literal("亮度"), 0.5f).setFillColor(0xFFFFC107)
        ...
        list.addItem(new ToggleItem(Component.literal("难度"),
                ...
                .setValueColor(0xFFFF5555).onChange(v -> System.out.println("难度变更")));
        topRemaining.addChild(list);
        topRow.addChild(topRemaining);
        topRow.addChild(new Panel(0, 0, 8, 0, 0).setFlexGrow(0));

        root.addChild(topRow);

        var gradientRow = new Panel(0, 0, sw, 22, 0).setLayout(Panel.LayoutDirection.HORIZONTAL, 6);
        gradientRow.addChild(new CustomButton(0, 0, 70, 22, Component.literal("⬇ 渐变"), ColorUtil.fromHex("#333333"))
                .setTextColor(0xFFFFFFFF).setBorderRadius(6)
                .setGradient(0xFF43A047, 0xFF1B5E20, CustomButton.GradientDirection.TOP_BOTTOM)
                .setGlow(0x40A5D6A7).withDefaultClickSound().withDefaultHoverSound()
        ...
        gradientRow.addChild(new CustomButton(0, 0, 80, 22, Component.literal("➡ 渐变"), ColorUtil.fromHex("#333333"))
                ...
                .setGlow(0x4042A5F5).withDefaultClickSound().withDefaultHoverSound()
        ...
        gradientRow.addChild(new CustomButton(0, 0, 94, 22, Component.literal("✨ 渐变+放大"), ColorUtil.fromHex("#333333"))
                ...
                .setGlow(0x40CE93D8).setHoverScale(1.10f).withDefaultClickSound().withDefaultHoverSound()
                .onClick(() -> System.out.println("渐变+放大组合按钮")));
        root.addChild(gradientRow);

        var dropdownRow = new Panel(0, 0, sw, 22, 0).setLayout(Panel.LayoutDirection.HORIZONTAL, 10);
        dropdownRow.addChild(new DropdownMenu<String>(0, 0, 130, 22,
                Component.literal("选择语言"), ColorUtil.fromHex("#444444"))
                .setTextColor(0xFFFFFFFF).setHoverItemColor(ColorUtil.fromHex("#5A5A5A"))
                .setDropdownBgColor(ColorUtil.fromHex("#2D2D2D")).setBorderColor(ColorUtil.fromHex("#888888"))
                .setItemHeight(18).setMaxVisibleItems(6)
        ...
        dropdownRow.addChild(new DropdownMenu<Integer>(0, 0, 100, 22,
                ...
                .setTextColor(0xFFFFFFFF).setHoverItemColor(ColorUtil.fromHex("#5A2A35"))
                .setDropdownBgColor(ColorUtil.fromHex("#2D1520")).setBorderColor(ColorUtil.fromHex("#9d2933"))
                .setItemHeight(18)
                .addOption(0, Component.literal("☀ 和平")).addOption(1, Component.literal("🌙 简单"))
                .addOption(2, Component.literal("⚡ 普通")).addOption(3, Component.literal("🔥 困难"))
                .setSelectedIndex(2).onSelect(v -> System.out.println("选中难度: " + v)));
        root.addChild(dropdownRow);

        var demoRow = new Panel(0, 0, sw, 150, 0).setLayout(Panel.LayoutDirection.HORIZONTAL, 10);
        var avatarPanel = new Panel(0, 0, 230, 150, ColorUtil.fromHex("#1A1A2E")).setAlpha(0.7f)
                .setLayout(Panel.LayoutDirection.VERTICAL, 6);
        avatarPanel.addChild(new Panel(0, 0, 0, 0, 0).setFlexGrow(1));
        var r1 = new Panel(0, 0, avatarPanel.width, 30, 0).setLayout(Panel.LayoutDirection.HORIZONTAL, 10);
        r1.addChild(new Avatar(0, 0, 28, "M").setBorder(ColorUtil.fromHex("#FFFFFF"), 2).setShape(Avatar.Shape.CIRCLE));
        ...
        var swapInfo = new Panel(0, 0, 0, 0, ColorUtil.fromHex("#2D2D2D")).setAlpha(0.5f);
        ...
        var swapSetting = new Panel(0, 0, 0, 0, ColorUtil.fromHex("#2D1F2A")).setAlpha(0.5f);
        ...
        var swap = new Swap(0, 0, 300, 150, ColorUtil.fromHex("#2A2A3E"))
                .addPage(swapInfo, Component.literal("信息")).addPage(swapSetting, Component.literal("设置"))
                .setActiveTabColor(ColorUtil.fromHex("#9d2933")).setInactiveTabColor(ColorUtil.fromHex("#3A3A4E"))
                .setTabHeight(18).setOnSwap(i -> System.out.println("切换到标签: " + i));
        demoRow.addChild(swap);
        demoRow.addChild(new Panel(0, 0, 0, 0, 0).setFlexGrow(1));
        root.addChild(demoRow);

        root.addChild(new Panel(0, 0, 0, 0, 0).setFlexGrow(1));

        var bottomPanel = new Panel(0, 0, sw, 34, ColorUtil.fromHex("#1a1a2e")).setAlpha(0.6f)
                .onClick(p -> System.out.println("状态栏面板被点击"));
        bottomPanel.addChild(new SimpleLabel(12, 8,
                Component.literal("状态栏 | 就绪 | %minecraft:diamond%    %minecraft:iron_ingot%    %minecraft:gold_ingot%"),
                0xFF88AAFF).setEnableItemIcons(true));
        root.addChild(bottomPanel);

        var bottomRow = new Panel(0, 0, sw, 18, 0).setLayout(Panel.LayoutDirection.HORIZONTAL, 6);
        ...
        var saveBtn = new CustomButton(0, 0, 64, 18, Component.literal("保存"), ColorUtil.fromHex("#9d2933"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#B33040")).setBorderRadius(6)
                .setGlow(0x60FFFFFF).setAlpha(0.95f).withDefaultClickSound()
        ...
        ref.toggle = new CustomButton(0, 0, 64, 18, Component.literal("禁用"), ColorUtil.fromHex("#555555"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#888888")).setBorderRadius(6)
                .setBorder(ColorUtil.fromHex("#999999"), 1).withDefaultClickSound()
                .onClick(() -> { boolean next = !saveBtn.isEnabled(); saveBtn.setEnabled(next); ref.toggle.setText(next ? Component.literal("禁用") : Component.literal("启用")); });
        bottomRow.addChild(ref.toggle);

        bottomRow.addChild(new CustomButton(0, 0, 64, 18, Component.literal("取消"), ColorUtil.fromHex("#555555"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#777777")).setBorderRadius(6)
                .setBorder(ColorUtil.fromHex("#888888"), 1).withDefaultClickSound()
                .onClick(() -> System.out.println("取消被点击")));

        var deleteModal = new Modal(Component.literal("确认删除"), Component.literal("确定要删除此项目吗？此操作无法撤销。"),
                () -> System.out.println("✓ 已确认删除"), () -> System.out.println("✗ 已取消删除")).setDialogWidth(280);
        addModal(deleteModal);

        bottomRow.addChild(new CustomButton(0, 0, 64, 18, Component.literal("删除"), ColorUtil.fromHex("#CC3333"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#FF4444")).setBorder(ColorUtil.fromHex("#FF6666"), 1)
                .withDefaultClickSound()
                .onClick(() -> { System.out.println("删除被点击 — 弹出对话框"); deleteModal.show(); }));

        bottomRow.addChild(new CustomButton(0, 0, 80, 18, Component.literal("✨ 放大"), ColorUtil.fromHex("#2E7D32"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#43A047")).setBorderRadius(8)
                .setGlow(0x40A5D6A7).setHoverScale(1.12f).withDefaultClickSound()
        ...
        bottomRow.addChild(new CustomButton(0, 0, 80, 18, Component.literal("⚙ 设置"), ColorUtil.fromHex("#ca6924"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#e29c45")).setBorderRadius(4)
                .setGlow(0x40A5D6A7).withDefaultClickSound()
        ...
        bottomRow.addChild(new CustomButton(0, 0, 100, 18,
                Component.literal("物品图标: %minecraft:diamond% %minecraft:iron_pickaxe%"), ColorUtil.fromHex("#2E7D32"))
                .setTextColor(0xFFFFFFFF).setHoverColor(ColorUtil.fromHex("#43A047")).setBorderRadius(4)
                .setBorder(ColorUtil.fromHex("#43A047"), 1).setAlpha(0.9f)
                .setEnableItemIcons(true)
                .onClick(() -> System.out.println("物品图标按钮被点击")));

        root.addChild(bottomRow);
        root.addChild(new Panel(0, 0, 0, 4, 0));
