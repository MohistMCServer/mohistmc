...
public class EscGui extends EnhancedScreen {
    ...
    protected void buildWidgets() {
        ...
        // 返回（顶部）
        addToolButton(toolPanel, btnX, topGap, "back",
        ...
        Runnable[] toolActions = {
                ...
                () -> minecraft.gui.setScreen(new ModListScreen(this)),
        ...
        for (int i = 0; i < toolNames.length; i++) {
            addToolButton(toolPanel, btnX, startY + i * (BTN_SZ + toolBtnGap),
                    toolNames[i], toolTips[i], toolActions[i]);
        }

        // 退出（底部）
        addToolButton(toolPanel, btnX, guiH - bottomGap - BTN_SZ, "exit",
        ...
        int nameX = (cw - Minecraft.getInstance().font.width(name)) / 2;
