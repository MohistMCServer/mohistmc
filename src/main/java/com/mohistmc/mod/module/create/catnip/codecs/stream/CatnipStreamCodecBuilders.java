package com.mohistmc.mod.module.create.catnip.codecs.stream;
...
public interface CatnipStreamCodecBuilders {
    ...
    /**
     * Encodes registry entries by their name instead of their network id.
     * <p>
     * {@link ByteBufCodecs#registry(ResourceKey)} requires the registry to be synced to the client,
     * which is not the case for custom built-in registries. These registries are registered
     * identically on both sides, so encoding the {@link Identifier} of each entry is sufficient.
     */
    static <T> StreamCodec<RegistryFriendlyByteBuf, T> registryEntry(Registry<T> registry) {
        return new StreamCodec<>() {
            @Override
            public T decode(RegistryFriendlyByteBuf buffer) {
                ...
                if (value == null) {
                    throw new IllegalStateException("Unknown entry in registry " + registry.key() + ": " + id);
                }
                return value;
            }
            ...
            public void encode(RegistryFriendlyByteBuf buffer, T value) {
                ...
                if (id == null) {
                    throw new IllegalStateException("Unregistered value in registry " + registry.key() + ": " + value);
                }
                Identifier.STREAM_CODEC.encode(buffer, id);
            }
        };
    }
}
