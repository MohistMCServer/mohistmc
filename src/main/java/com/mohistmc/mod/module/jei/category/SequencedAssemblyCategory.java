...
import com.mohistmc.mod.module.create.AllAssemblyRecipeNames;
...
import com.mohistmc.mod.module.jei.widget.JunkChanceTooltip;
...
public class SequencedAssemblyCategory extends CreateCategory<RecipeHolder<SequencedAssemblyRecipe>> {
    ...
    public static class FillingRenderer extends SequencedRenderer<FillingRecipe> {
        @Override
        public void render(GuiGraphicsExtractor graphics, int i, int x, int y, Optional<IRecipeSlotView> slot) {
            slot.flatMap(s -> s.getDisplayedIngredient(NeoForgeTypes.FLUID_STACK)).ifPresent(fluidStack -> {
                float scale = 35 / 46f;
                Matrix3x2fStack matrices = graphics.pose();
                matrices.pushMatrix();
                matrices.translate(x, y);
                ...
                DataComponentPatch components = fluidStack.getComponentsPatch();
        ...
        public Component getSequenceName(FillingRecipe recipe, Optional<IRecipeSlotView> slot) {
            Component name = slot.flatMap(s -> s.getDisplayedIngredient(NeoForgeTypes.FLUID_STACK)).map(fluidStack -> {
                Fluid fluid = fluidStack.getFluid();
                if (fluid == AllFluids.POTION) {
                    DataComponentMap components = fluidStack.getComponents();
